...
main(int argc, char *argv[])
{
    ...
    hg_ret = hg_test_bulk_null(
        info.hg_class, info.context, info.request_class, info.target_addr);
    HG_TEST_CHECK_ERROR(
    ...
    hg_ret = hg_test_bulk_contig(info.hg_class, info.context,
        info.request_class, 0, 0, info.target_addr, buf_size, 0, 0, 0);
    ...
    hg_ret = hg_test_bulk_contig(info.hg_class, info.context,
        info.request_class, 0, 0, info.target_addr, buf_size, buf_size, 0, 0);
    HG_TEST_CHECK_ERROR(hg_ret != HG_SUCCESS, done, ret, EXIT_FAILURE,
    ...
    hg_ret =
        hg_test_bulk_contig(info.hg_class, info.context, info.request_class, 0,
            0, info.target_addr, buf_size, buf_size / 4, buf_size / 2 + 1, 0);
    HG_TEST_CHECK_ERROR(hg_ret != HG_SUCCESS, done, ret, EXIT_FAILURE,
        "contiguous RPC bulk failed");
    HG_PASSED();
    ...
    HG_TEST("contiguous RPC bulk (size BUFSIZE/8, offsets BUFSIZE/2 + 1, "
            "BUFSIZE/4)");
    hg_ret = hg_test_bulk_contig(info.hg_class, info.context,
        info.request_class, 0, 0, info.target_addr, buf_size, buf_size / 8,
        buf_size / 2 + 1, buf_size / 4);
    HG_TEST_CHECK_ERROR(hg_ret != HG_SUCCESS, done, ret, EXIT_FAILURE,
        "contiguous RPC bulk failed");
    HG_PASSED();
    ...
    hg_ret = hg_test_bulk_small(info.hg_class, info.context, info.request_class,
        info.target_addr, 8, 0, 0);
    HG_TEST_CHECK_ERROR(hg_ret != HG_SUCCESS, done, ret, EXIT_FAILURE,
    ...
    hg_ret = hg_test_bulk_small(info.hg_class, info.context, info.request_class,
        info.target_addr, 8, 4, 2);
    HG_TEST_CHECK_ERROR(hg_ret != HG_SUCCESS, done, ret, EXIT_FAILURE,
        "small segmented RPC bulk failed");
    HG_PASSED();

    HG_TEST("segmented RPC bulk (size BUFSIZE, offsets 0, 0)");
    hg_ret = hg_test_bulk_seg(info.hg_class, info.context, info.request_class,
        info.target_addr, buf_size, buf_size, 0, 0, 16);
    HG_TEST_CHECK_ERROR(hg_ret != HG_SUCCESS, done, ret, EXIT_FAILURE,
    ...
    hg_ret = hg_test_bulk_seg(info.hg_class, info.context, info.request_class,
        info.target_addr, buf_size, buf_size / 4, buf_size / 2 + 1, 0, 16);
    HG_TEST_CHECK_ERROR(hg_ret != HG_SUCCESS, done, ret, EXIT_FAILURE,
        "segmented RPC bulk failed");
    HG_PASSED();
    ...
    HG_TEST("segmented RPC bulk (size BUFSIZE/8, offsets BUFSIZE/2 + 1, "
            "BUFSIZE/4)");
    hg_ret = hg_test_bulk_seg(info.hg_class, info.context, info.request_class,
        info.target_addr, buf_size, buf_size / 8, buf_size / 2 + 1,
        buf_size / 4, 16);
    HG_TEST_CHECK_ERROR(hg_ret != HG_SUCCESS, done, ret, EXIT_FAILURE,
        "segmented RPC bulk failed");
    HG_PASSED();
    ...
    hg_ret = hg_test_bulk_seg(info.hg_class, info.context, info.request_class,
        info.target_addr, buf_size, buf_size, 0, 0, 1024);
    HG_TEST_CHECK_ERROR(hg_ret != HG_SUCCESS, done, ret, EXIT_FAILURE,
    ...
    hg_ret = hg_test_bulk_seg(info.hg_class, info.context, info.request_class,
        info.target_addr, buf_size, buf_size / 4, buf_size / 2 + 1, 0, 1024);
    HG_TEST_CHECK_ERROR(hg_ret != HG_SUCCESS, done, ret, EXIT_FAILURE,
        "over-segmented RPC bulk failed");
    HG_PASSED();
    ...
    if (strcmp(HG_Class_get_name(info.hg_class), "ofi") == 0 ||
        strcmp(HG_Class_get_name(info.hg_class), "na") == 0 ||
        strcmp(HG_Class_get_name(info.hg_class), "ucx") == 0) {
        HG_TEST("bind contiguous RPC bulk (size BUFSIZE, offsets 0, 0)");
        hg_ret =
            hg_test_bulk_contig(info.hg_class, info.context, info.request_class,
                1, 0, info.target_addr, buf_size, buf_size, 0, 0);
        HG_TEST_CHECK_ERROR(hg_ret != HG_SUCCESS, done, ret, EXIT_FAILURE,
            "bind contiguous RPC bulk failed");
        HG_PASSED();

        HG_TEST("forward bind contiguous RPC bulk (size BUFSIZE, offsets "
                "0, 0)");
        hg_ret = hg_test_bulk_contig(info.hg_class, info.context,
            info.request_class, 1, 1, info.target_addr, 3584, 3584 / 4, 0, 0);
        HG_TEST_CHECK_ERROR(hg_ret != HG_SUCCESS, done, ret, EXIT_FAILURE,
            "bind contiguous RPC bulk failed");
        HG_PASSED();
    }
