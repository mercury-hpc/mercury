...
HG_Test_init(int argc, char *argv[], struct hg_test_info *hg_test_info)
{
    ...
    /* Multi-recv */
    if (hg_test_info->na_test_info.multi_recv)
        hg_init_info.use_multi_recv = HG_TRUE;
    ...
    */
    ...
    if (hg_test_info->na_test_info.listen) {
        ret = hg_test_self_addr_publish(hg_test_info->hg_class, false);
        HG_TEST_CHECK_HG_ERROR(done, ret, "hg_test_self_addr_publish() failed");
