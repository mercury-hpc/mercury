...
#define HG_GEN_DECL_FUNC_PARAMS(with_input, in_params, extra_in_params, \
        ...
        BOOST_PP_SEQ_TO_TUPLE( \
                BOOST_PP_IF(BOOST_PP_OR(with_input, with_output), \
                        HG_GEN_DECL_FUNC_PARAM_SEQ(0, in_params) \
                        HG_GEN_DECL_FUNC_PARAM_SEQ(0, extra_in_params) \
                        HG_GEN_DECL_FUNC_PARAM_SEQ(1, out_params) \
...
#define HG_GEN_FUNC_PARAMS(with_input, in_params, extra_in_params, \
        ...
        BOOST_PP_SEQ_TO_TUPLE( \
                BOOST_PP_IF(BOOST_PP_OR(with_input, with_output), \
                        HG_GEN_FUNC_PARAM_SEQ(0, in_params) \
                        HG_GEN_FUNC_PARAM_SEQ(0, extra_in_params) \
                        HG_GEN_FUNC_PARAM_SEQ(1, out_params) \
...
#define HG_BULK_REGISTER(handle, bulk_handle, with_ret, fail_ret, bulk_read) \
        hg_ret = HG_Bulk_create(HG_Get_info(handle)->hg_bulk_class, \
                1, &HG_GEN_GET_NAME(BOOST_PP_SEQ_HEAD(HG_BULK_BUF)),\
...
#define HG_BULK_LOCAL_ALLOCATE(origin_bulk_handle, local_bulk_handle) \
        ...
        HG_Bulk_create(HG_Get_info(handle)->hg_bulk_class, \
                1, &HG_GEN_GET_NAME(BOOST_PP_SEQ_HEAD(HG_BULK_BUF)), \
                &HG_GEN_GET_NAME(BOOST_PP_SEQ_HEAD(HG_BULK_COUNT)), \
                HG_BULK_READWRITE, \
                &local_bulk_handle);
        ...
        { \
            ...
            BOOST_PP_IF(with_bulk, \
                    HG_BULK_FREE(HG_GEN_GET_NAME(BOOST_PP_SEQ_HEAD(HG_BULK_PARAM)), \
                            with_ret, ret_fail), BOOST_PP_EMPTY()) \
            \
            /* Get output */ \
            BOOST_PP_IF(BOOST_PP_OR(with_output, with_ret), \
...
#define MERCURY_GEN_CALLBACK_STUB(gen_func_name, func_name, \
        with_ret, ret_type, \
        with_input, in_struct_type_name, in_params, \
        with_output, out_struct_type_name, out_params, \
        with_bulk, bulk_read, \
        ...
        { \
                ...
                BOOST_PP_IF(with_bulk, \
                        BOOST_PP_IF(bulk_read, \
                                BOOST_PP_EMPTY(), HG_BULK_TRANSFER(handle, \
                                        HG_GEN_GET_NAME(BOOST_PP_SEQ_HEAD(HG_BULK_PARAM)), \
                                        HG_GEN_GET_NAME(BOOST_PP_SEQ_HEAD(HG_BULK_LOCAL_PARAM)), \
                                        bulk_read)), \
                        BOOST_PP_EMPTY()) \
                ...
                /* Free bulk handle */ \
                BOOST_PP_IF(with_bulk, \
                        HG_BULK_LOCAL_FREE(HG_GEN_GET_NAME(BOOST_PP_SEQ_HEAD(HG_BULK_LOCAL_PARAM))), \
                        BOOST_PP_EMPTY()) \
                \
