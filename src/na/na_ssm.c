...
int addr_parser(char *str, na_ssm_destinfo_t *addr)
{
    sscanf(str, "%15s://%63[^:]:%d", addr->proto, addr->hostname, port);
    return 0;
}
...
static int na_ssm_addr_lookup(const char *name, na_addr_t *addr)
{
    ...
    ssm_Iaddr ssm_addr->addrs = ssm_addr(ssm);
    ssm_Haddr ssm_addr->addr = addrs->create(addrs, &addrargs);
...
//            BMI_EXT_ALLOC, bmi_tag, bmi_request, bmi_context, NULL);
    ...
    na_ssm_mem_handle_t *mem_handle = (na_ssm_msm_handle_t *)malloc(sizeof(na_ssm_mem_handle_t));

    ssm_cb_t cb = {
        .pcb = msg_send_cb,
        .cbdata = mem_handle,
    };
    mem_handle->mr = ssm_mr_create(NULL, buf, ssm_buf_size);
    ssm_ret = ssm_put(ssm, ssm_peer_addr, mem_handle->mr, NULL, ssm_tag, &cb, SSM_NOF);
    if (ssm_ret < 0) {
        ...
        na_mem_handle_t *mem_handle)
{
    ...
    mr = ssm_create_mr(NULL, buf, buf_size);
    pssm_mr->mr = mr;
...
{
    int ret = NA_SUCCESS;
    na_ss_mem_handle_t *ssm_mem_handle;
