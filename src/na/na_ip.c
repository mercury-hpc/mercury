    ...
    struct sockaddr_in **sin_addr_ptr)
{
    ...
    struct sockaddr_in *sin_addr = NULL;
    ...
    sin_addr = calloc(1, sizeof(*sin_addr));
    NA_CHECK_SUBSYS_ERROR(cls, sin_addr == NULL, done, ret, NA_NOMEM,
        "Could not allocate sin address");
    sin_addr->sin_family = AF_INET;
    sin_addr->sin_port = htons(port & 0xffff);

    /* Try to resolve hostname first so that we can later compare the IP */
    memset(&hints, 0, sizeof(struct addrinfo));
    hints.ai_family = sin_addr->sin_family;
    ...
    if (!found || ret != NA_SUCCESS || !sin_addr_ptr)
        free(sin_addr);
    freeifaddrs(ifaddrs);
    if (hostname_res)
