    ...
    struct fi_info **fi_info_p)
{
    ...
    if (info) {
        /* Set appropriate limits if we are listening (limit to tcp and cxi) */
        if (prov_type == NA_OFI_PROV_TCP || prov_type == NA_OFI_PROV_CXI) {
            if (info->listen) {
                /* Listeners may end up with a large number of outstanding sends
                 * for RPC responses */
                hints->tx_attr->size = NA_OFI_TX_SIZE_LISTENER;
                ...
                hints->tx_attr->size = NA_OFI_TX_SIZE;
                hints->rx_attr->size = NA_OFI_RX_SIZE;
            }
        }
