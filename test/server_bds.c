...
int fs_bla_write(hg_handle_t handle)
{
    ...
    ret = HG_Handler_get_input(handle, &bla_write_in_buf, &bla_write_in_buf_size);
    if (ret != HG_SUCCESS) {
    ...
    HG_Bulk_block_handle_create(bla_write_buf, bla_write_nbytes, HG_BULK_READWRITE, &bla_write_bulk_block_handle);
   
    /* Read bulk data here and wait for the data to be here  */ 
    ret = HG_Bulk_read(bla_write_bulk_handle, source, bla_write_bulk_block_handle);
    if (ret != HG_SUCCESS) {
        fprintf(stderr, "Could not read bulk data\n");
        return ret;
    }

    ret = HG_Bulk_wait(bla_write_bulk_block_handle, HG_BULK_MAX_IDLE_TIME);
    if (ret != HG_SUCCESS) {
    ...
    HG_Handler_get_output(handle, &bla_write_out_buf, &bla_write_out_buf_size);
...
int main(int argc, char *argv[])
{
    ...
    /* Register routine */
    MERCURY_HANDLER_REGISTER("bla_write", fs_bla_write);
